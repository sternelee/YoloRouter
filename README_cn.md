...
> **首次使用？** 参阅 [QUICK_INSTALL.txt](QUICK_INSTALL.txt) 获取 5 分钟快速开始指南
...
参阅 [INSTALL.md](INSTALL.md) 了解完整的环境变量设置说明。
...
| 脚本 | 功能 | 使用场景 |
|------|------|---------|
| **install.sh** | 一键安装（支持多平台） | 首次安装，自动检查依赖和编译 |
| **uninstall.sh** | 安全卸载 | 完全移除 YoloRouter（保留配置）|
| **yolo-setup.sh** | 交互式配置向导 | 初始配置、重新配置环境变量、测试连接 |
...
详见 [INSTALL.md](INSTALL.md) 了解完整说明。
...
- **[README.md](README.md)** - English version of README
- **[00-START-HERE.md](00-START-HERE.md)** - 文档导航指南（根据角色推荐）
- **[QUICK_INSTALL.txt](QUICK_INSTALL.txt)** - 5 分钟快速开始指南
- **[INSTALL.md](INSTALL.md)** - 详细安装指南（支持 macOS、Linux、Windows）
- **[USER_GUIDE.md](USER_GUIDE.md)** - 完整的用户指南（配置、API 使用、故障排除）

### 🔧 开发者和运维文档

- **[RELEASE_GUIDE.md](RELEASE_GUIDE.md)** - 发布流程指南（含自动化工作流）
- **[CI_CD_GUIDE.md](CI_CD_GUIDE.md)** - CI/CD 快速参考
- **[PROJECT_SUMMARY.md](PROJECT_SUMMARY.md)** - 项目架构总结与技术选择
...
- **[config.example.toml](config.example.toml)** - 配置示例（所有供应商和场景）

### 🔄 GitHub Actions 工作流

| 工作流 | 文件 | 功能 |
|--------|------|------|
| **发布** | `.github/workflows/release.yml` | 多平台构建、发布创建、文档发布 |
| **持续集成** | `.github/workflows/ci.yml` | 代码质量、测试、安全扫描 |
| **开发构建** | `.github/workflows/build.yml` | 开发分支的跨平台构建 |
| **验证** | `.github/workflows/validate.yml` | 工作流文件语法验证 |
...
- [x] 监控和统计
- [x] TUI 认证
- [x] **跨平台安装脚本** (macOS, Linux, Windows)
- [x] **GitHub Actions CI/CD 工作流** (自动多平台构建、发布)
- [x] **完整安装文档** (快速开始、详细指南、故障排除)
- [ ] 配置热重载
- [ ] 数据库持久化
- [ ] Prometheus metrics
- [ ] Kubernetes 部署
...
**A**: 是的！该项目已经过充分测试（54 个测试全通过），包含完整的错误处理、监控和自动化发布流程。参阅 [RELEASE_GUIDE.md](RELEASE_GUIDE.md) 了解发布过程。

### Q: 安装有多简单？

**A**: 非常简单！运行 `bash install.sh` 即可一键安装。支持 macOS、Linux 和 Windows。详见 [QUICK_INSTALL.txt](QUICK_INSTALL.txt)。

### Q: 如何配置？

**A**: 有两种方式：
1. **推荐**：运行 `bash yolo-setup.sh` 交互式配置
2. **手动**：编辑 `config.toml` 文件

详见 [INSTALL.md](INSTALL.md) 了解完整说明。

### Q: 如何卸载？

**A**: 运行 `bash uninstall.sh`，按提示操作。配置文件可保留或删除。
